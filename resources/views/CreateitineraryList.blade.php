...
<div class="card-body">
    ...
    <form>
        <div class="form-group row">
            ...
            <div class="col-sm-9">
                <input type="text"  class="form-select input-style" id="authorInput"
        ...
        <div class="form-group row">
            <label for="dateForApplication" class="col-sm-3 col-form-label">Дата выполнения</label>
            <div class="col-sm-9">
                <input type="date" name="dateForApplication"
                    class="form-select input-style" id="dateForApplication" required>
            </div>
        </div>
    ...
    <div class="form-group row">
        ...
        <div class="col-sm-9">
            <input class="form-select input-style" id="timeinput"
                placeholder="Введите время формата Ч:М:C" value="">
        </div>
    </div>
                    ...
                    $("#fillButton").on("click", function () {
                        $.ajax({
                            ...
                            success: function (response) {
                                // Обработайте данные, полученные от сервера
                                if (response.success) {
                                    // Переберите данные и выполните действия для каждой записи
                                    response.data.forEach(function (application) {
                                        // Создайте новую строку и заполните ее данными из application
                                        var newRow = $("<tr>");
                                        var rowCount = $('#applicationsTable tbody tr').length + 1;
                                        newRow.append('<td>' + rowCount + '</td>');
                                        ...
                                        if (typeof application.productsInfo === 'string') {
                                            ...
                                            // Получаем ключи объекта и объединяем их в строку
                                            var keysString = Object.keys(productsInfoObject).join(', ');
                                            // Добавляем ячейку с ключами в строку
                                            newRow.append('<td>' + keysString + '</td>');
                                        } else {
                                            newRow.append('<td></td>');
                                        }

                                        // заполнение таблицы по полученным данным с ajax 
                                        newRow.append('<td>' + '</td>');
