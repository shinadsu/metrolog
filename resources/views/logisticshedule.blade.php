            ...
            <div class="calendar-container">
                ...
                @foreach ($users as $user)
                  ...
                  <tr>
                      ...
                      @for ($day = 0; $day < $daysToShow; $day++)
                          ...
                          <td class="text-center{{ $isScheduledIcon ? ' scheduled' : '' }} {{ $date->dayOfWeek == Carbon::SATURDAY ? ' saturday' : ($date->dayOfWeek == Carbon::SUNDAY ? ' sunday' : '') }}" data-logist-id="{{ $user->id }}" data-uniq-identefy="{{ $uniqIdentefy }}" data-date="{{ $formattedDate }}" data-is-scheduled="{{ $isScheduledIcon ? 1 : 0 }}">
                              {{ $isScheduledIcon }}
                          </td>
                      @endfor
              ...
              @endforeach
                  ...
                  <tr>
                    ...
                    <td class="text-center">{{ $events->where('is_scheduled', '*')->count() }}</td> <!-- Новая ячейка -->
                </tr>
                </tbody>
              </table>
            </div>
